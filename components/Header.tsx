...
export const Header: React.FC<HeaderProps> = ({ onStartDemo, onStopDemo, isDemoRunning }) => {
  return (
    <header className="bg-white dark:bg-gray-800 shadow-md">
      <div className="container mx-auto px-4 lg:px-6 py-3 flex items-center justify-between">
        ...
        >
            {isDemoRunning ? <StopIcon className="w-5 h-5" /> : <PlayIcon className="w-5 h-5" />}
            <span className="hidden sm:inline">{isDemoRunning ? "Stop Demo" : "Start Demo"}</span>
        </button>
      </div>
    </header>
  );
